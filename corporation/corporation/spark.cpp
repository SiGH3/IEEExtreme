...
void Query(const vector<long long>& cur_salary, const vector<int>& happiness, int t, int l, int r, vector<pair<long long, long long>>& result)
{
    ...
    if (B == 0) throw invalid_argument("Division by zero in query");
...
int main()
{
    int N, Q;
    cin >> N >> Q;
    if (N <= 0 || Q <= 0) throw invalid_argument("Invalid input size");
    ...
    while (Q--)
    {
        ...
        {
            cin >> l >> r >> c;
            if (l < 1 || r > N || l > r) throw out_of_range("Invalid range");
        ...
        else throw invalid_argument("Invalid operation type");
    }
